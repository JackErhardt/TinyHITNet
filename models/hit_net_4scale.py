...
class FeatureExtractor(nn.Module):
    ...
    def forward(self, input):
        ...
        # print(">> down_X0.shape: {}".format(x0.shape))
        # print(">> down_X1b.shape: {}".format(x1.shape))
        # print(">> down_X2b.shape: {}".format(x2.shape))
        # print(">> down_X3b.shape: {}".format(x3.shape))
        # print(">> down_X3d.shape: {}".format(o0.shape))
        # print(">> up_X3e.shape: {}".format(o1.shape))
        # print(">> up_X2e.shape: {}".format(o2.shape))
        # print(">> up_X1e.shape: {}".format(o3.shape))
        # print(">> up_X0e.shape: {}".format(o4.shape))
...
def warp_and_aggregate(hyp, left, right):
    ...
    # print(">>>> waaX_disp_up.shape: {}".format(d_expand.shape))
    ...
    # print(">>>> waaX_d_range.shape: {}".format(d_range.shape))
...
class LevalProp(nn.Module):
    ...
    def forward(self, hyps, l, r):
        ...
        # for h in cost:
            # print(">>> hypX.shape: {}".format(h.shape))
        cost = torch.cat(cost, dim=1)
        # print(">>> propX_cat0.shape: {}".format(cost.shape))
...
class LevelInit(nn.Module):
    ...
    def forward(self, l, r, ref=None):
        ...
        # print(">>> initX_lreduce.shape: {}".format(lt.shape))
        lt = self.conv_em(lt)
        # print(">>> initX_lem.shape: {}".format(lt.shape))
        ...
        # print(">>> initX_rreduce.shape: {}".format(rt.shape))
        rt = self.conv_em(rt)

        cv = make_cost_volume_v2(lt, rt, self.max_disp)
        cv = torch.norm(cv, p=1, dim=1)
        cv_min, d = torch.min(cv, dim=1, keepdim=True)
        # print(">>> initX_cv_min.shape: {}".format(cv_min.shape))
        # print(">>> initX_d.shape: {}".format(d.shape))
        d = d.float()

        if ref is None:
            ref = lt
        p = torch.cat((cv_min, ref), dim=1)
        # print(">>> initX_cat0.shape: {}".format(p.shape))
        p = self.conv_hyp(p)
        # print(">>> initX_hyp.shape: {}".format(p.shape))
        p = torch.cat((d, torch.zeros_like(d), torch.zeros_like(d), p), dim=1)
        # print(">>> initX_cat1.shape: {}".format(p.shape))
        return p, cv
...
class Level(nn.Module):
    ...
    def forward(self, l, r, h=None, ref=None):
        ...
        # print(">> hi.shape: {}".format(hi.shape))
        ...
        if self.h_size == 1:
            ...
            # print(">> h_out.shape: {}".format(h.shape))

            return h, cv, hi[:, :1], []
        else:
            # print(">> h_in.shape: {}".format(h.shape))
            # print(">> Running LevalProp")
